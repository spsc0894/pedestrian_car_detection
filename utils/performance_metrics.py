...
class PerformanceMetrics:
    ...
    def update(self, last_request_start_time, frame, bike,car,pedestrians,position=(15, 30),
        ...
        if current_fps is not None:
            put_highlighted_text(frame, "FPS: {:.1f}".format(current_fps),
                                 (position[0], position[1]), cv2.FONT_HERSHEY_COMPLEX, font_scale, color, thickness)
        put_highlighted_text(frame, "P: {:.1f}".format(pedestrians),
                                 (position[0], position[1]+30), cv2.FONT_HERSHEY_COMPLEX, font_scale, color, thickness)
        put_highlighted_text(frame, "C: {:.1f}".format(car),
                                 (position[0], position[1]+60), cv2.FONT_HERSHEY_COMPLEX, font_scale, color, thickness)
        put_highlighted_text(frame, "B: {:.1f}".format(bike),
                                 (position[0], position[1]+90), cv2.FONT_HERSHEY_COMPLEX, font_scale, color, thickness)
